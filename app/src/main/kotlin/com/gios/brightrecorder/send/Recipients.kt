package com.gios.brightrecorder.send

/*
 * Ported from Roll's `send/` package rather than rewritten, and deliberately kept close enough
 * to it that a fix in one is obviously the same fix in the other. Two apps on this phone now
 * ask "who?" instead of "which app?", and the hard parts of that question -- normalising a
 * phone number written five ways, deciding what a two-letter query matches -- are the same
 * hard parts in both. When a third app needs it, this is what moves into light-common.
 */
...
data class Recipient(
    ...
    /**
     * The address a recording should go to: a phone if there is one, otherwise the best
...
object Recipients {
     ...
     * by [key] and ordered phones-before-emails, since this exists to send a recording to a
     ...
     * once is still somebody you might send a moment to, and dropping it would make the picker
     * quietly incomplete.
     */
    fun merge(rows: List<Row>): List<Recipient> {
        ...
        for ((id, group) in byContact) {
            ...
            // phone, then email. A recording sent to a landline is a recording nobody hears.
            val addresses = group
                .sortedWith(
                    compareBy(
     ...
     * itself after a phone swap, which is the same reasoning behind keying a clip by its
     * filename rather than by a row id.
     *
